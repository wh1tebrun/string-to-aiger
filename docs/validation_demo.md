...
The current full profile reports 31 scripts and 311 explicit test functions. The internal profile reports 24 scripts and 267 functions; it still requires Bash for the manual fake-simulator regression. The [internal workflow](../.github/workflows/internal-tests.yml) runs this profile on Ubuntu with Python 3.10 and 3.12, while the real external-tool checks remain part of the provisioned full release gate.
